#include "AStarPathAgent.h"

namespace AStarPathfinding
{
    void AStarPathAgent::SetNode(Node* node)
    {
        currentNode = node;
        position = node->position;
    }

    void AStarPathAgent::Update(float deltaTime)
    {
    ...
    void AStarPathAgent::GoToNode(Node* node)
    {
        path = AStarSearch(currentNode, node);
        currentIndex = 0;
    }

    void AStarPathAgent::Draw()
    {
